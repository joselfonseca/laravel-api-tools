...
class JsonResponder implements ResponderInterface
{
     ...
     * @return Illuminate\Support\Facades\Response
     */
    public function appError($errorCode = "Exception",
                             $errorDescription = "The app encounter an error not defined")
    {
        $this->response = $this->laravelResponse->setContent(json_encode(['ErrorCode' => $errorCode,
                'ErrorDescription' => $errorDescription]))->setStatusCode(500);
        return $this->respond();
    }
    ...
    {
        $this->response = $this->laravelResponse->setStatusCode(201);
        return $this->respond();
    }
}